...
description: "Manizales de Pie uses 10 PostgreSQL tables with PostGIS, row-level security, security_invoker views, and proximity RPCs. Reference the full schema here."
---
...
† Added in a later migration, not in `20260813000000_init.sql`: `census_point` in `20260814040000_site_type_census_point.sql`; `attended` in `20260815020000_work_order_update_kinds.sql`; `home_stay` in `20260814130000_resource_offer_public.sql`.
...
| `animal_report` | Lost, found, or sighted animals with a photo URL, species, description, and contact. The location is the last-seen point (nullable) — a lost animal has no fixed location. |
