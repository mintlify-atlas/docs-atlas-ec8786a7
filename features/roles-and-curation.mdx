...
description: "Three roles govern what actions are available in Manizales de Pie: visitor, contributor, and curator. Curators moderate content using the admin actions strip on every entity card."
---
...
<CardGroup cols={3}>
  <Card title="Edit" icon="pencil">
    Opens an inline edit form for the entity's text fields (name, description, address, schedule, WhatsApp). Fields differ per entity; the strip itself does not.
  </Card>
...
The strip's state and label copy all come from `ADMIN_LABEL` in `lib/labels.ts`. The vocabulary — edit, hide, publish, delete — is identical across all entity types by design.
...
Only someone with direct database access can elevate a user to curator. The `20260814010000_bootstrap_curator.sql` migration shows the pattern for the first curator: it rewrites the `handle_new_user` trigger to assign `curator` to a specific email address at account-creation time. The address is in the repo by design — it grants nothing to whoever reads it, because the account still has to pass Google OAuth. What it does buy is an auditable answer to "who made themselves curator, and when."
